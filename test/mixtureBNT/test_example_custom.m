...
discrete_nodes = [1 2];
 ...
 %take just first 50 for training
 
 trainingX=[normalX; abnormalX];

% labeling
 trainingC(1:size(normalX,1)) = 1;   %% Class 1 is walking
 trainingC(size(normalX,1)+1:size(normalX,1)+size(abnormalX,1)) = 2; %% Class 2 is running
 
 
 
 testX = [s3;s6]   %% The first is a wnormal sequence and then an abnormal ones
